...
def train_entities(intents: list, save_path: str, batch_size=64, learning_rate=1E-4, epochs=200):
    expand_all_examples(intents)
    intents_hash = hash_intents(intents)
    if path.exists(save_path):
        old_data: dict = torch.load(save_path)
    ...
    print("device" + PYTORCH_DEVICE.__str__())
    model = TokenClassificationNet(input_size, embed_dim,
    ...
    for epoch in range(epochs):
        ...
        for idx, data in enumerate(train_loader):
            ...
            outputs = model(text)
            loss = criterion(
                outputs.view(-1, outputs.size(-1)), entities.view(-1))

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            # Compute and return accuracy
            total_accu += (outputs.argmax(dim=2) ==
                           entities).sum().item() / entities.flatten().shape[0]
